...
var loadingDialogInstance = (function () {
    ...
    instance.openSubscribe = function (subscriber) {
...
function subscribeDialog (subscriber) {
    loadingDialogInstance.openSubscribe(subscriber);
}
function unsubscribeDialog (subscriber) {
...
function psAlert (options, details, callback) {
	...
	if (typeof options.message === 'string') {
		...
		if (options.details) {
			...
			var showDetails = $j('<a/>', {href: '', text: pss_text('psx.js.scripts.psdialog.show_details')}).appendTo(options.content);
			var hideDetails = $j('<a/>', {href: '', text: pss_text('psx.js.scripts.psdialog.hide_details')}).appendTo(options.content).hide();
			$j('<br/>').appendTo(options.content);
...
function psDialog (options) {
	...
	if (!options.dialogDiv) {
		...
		if (options.dialogDiv.length === 0) {
			options.dialogDiv = $j('<div/>', {id: options.docked?'psDialogDocked':'psDialog', 'class': 'psDialog'}).appendTo('body');
		}
	} else {
	...
	if(options.docked){
		...
		options.dialogDiv.dialog({beforeClose:function(event, ui){
			...
			if(!event.isDefaultPrevented() && skipSlideIn !== true && !psDialogStickyOpen.isAlreadyOpen()){
				...
				var animationOptions = {duration:600};
		...
		$j('#psDialogDocked :input:visible:enabled:first').focus();
...
function toggleDrawerPanel(panelId, modalOverlayId, state) {
    if (state === 'expanded') {
        ...
        $j('#'+panelId+' :input:visible:enabled:first').focus();
    }
